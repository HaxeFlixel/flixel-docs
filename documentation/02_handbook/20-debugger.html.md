...
Note that the debugger does not exist when compiling with `FLX_NO_DEBUG`. With the default `Project.xml`, this is the case in release mode. If you are using lime add the `-debug` flag to your command to avoid release mode (e.g. `lime test neko -debug`)
